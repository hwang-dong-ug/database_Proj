...
void usetofree(off_t wbf) // off_t에 해당하는 page를 free page로 변경
{
...
char *db_find(int64_t key)
{
    ...
    {
        if (p->records[i].key == key) // 일치하는 것을 찾으면 break
            break;
    }
    if (i == p->num_of_keys) // 못찾은 경우 null 반환
    {
        free(p);
        return NULL;
    }
    else
    {
        ...
        free(p);
        return value; // 키에 해당하는 value를 return (찾은 경우)
    }
}
...
int db_insert(int64_t key, char *value)
{
    ...
    if (rt == NULL) // 비어있는 Tree 이면
    {
        start_new_file(nr); // 새로운 page를 만들어준다
        return 0;
    }

    char *dupcheck;
    dupcheck = db_find(key);
    if (dupcheck != NULL) // 이미 해당 키에 해당하는 value가 존재 하므로 return -1
    {
        free(dupcheck);
        return -1;
    }
    ...
    if (leafp->num_of_keys < LEAF_MAX) // record가 들어갈 leaf page에 공간이 충분하면
    {
        insert_into_leaf(leaf, nr); // 그냥 insert
        free(leafp);
        return 0;
    }

    insert_into_leaf_as(leaf, nr); // 새로운 page를 만들고 해당 page에 record insert
    free(leafp);
    // why double free?
    return 0;
}
...
off_t insert_into_leaf(off_t leaf, record inst)
{
    ...
    insertion_point = 0;
    while (insertion_point < p->num_of_keys && p->records[insertion_point].key < inst.key) // record가 들어 갈 자리를 insertion_point로 찾는다
    {
        insertion_point++;
    }
    for (i = p->num_of_keys; i > insertion_point; i--) // record가 들어갈 공간을 마련하기 위해서 한칸씩 밀어내는 작업
    {
        p->records[i] = p->records[i - 1];
    }
    p->records[insertion_point] = inst;
    p->num_of_keys++;
    pwrite(fd, p, sizeof(page), leaf); // 바뀐 page를 저장
...
off_t insert_into_leaf_as(off_t leaf, record inst)
{
    ...
    page *nl = load_page(new_leaf); //새로운 page를 만들고
    nl->is_leaf = 1;
    temp = (record *)calloc(LEAF_MAX + 1, sizeof(record)); // leaf max 보다 1개 더 큰 size를 임시로
    if (temp == NULL)
    {
    ...
    insertion_index = 0;
    page *ol = load_page(leaf); // 가득차서 넣지 못했던 ol의 page pointer
    while (insertion_index < LEAF_MAX && ol->records[insertion_index].key < inst.key)
    {
        insertion_index++;
    }
    for (i = 0, j = 0; i < ol->num_of_keys; i++, j++) // 316 - 322 들어갈 자리 보다 더 큰 key 해당하는 record를 한 칸씩 밀어낸다
    {
    ...
    ol->num_of_keys = 0; // 326에서 수정할 것을 대비해 num_of_keys를 0으로 초기화
    split = cut(LEAF_MAX);

    for (i = 0; i < split; i++) // 326 - 336 반절은 ol에 반절은 nl 나누어 삽입
    {
        ol->records[i] = temp[i];
        ol->num_of_keys++;
    }
    ...
    nl->next_offset = ol->next_offset; // linked list insert와 유사하게 offset을 변경
    ol->next_offset = new_leaf;

    for (i = ol->num_of_keys; i < LEAF_MAX; i++) // 344-355 위에서 split 할때 못했던 뒷처리
    {
        ol->records[i].key = 0;
    ...
    nl->parent_page_offset = ol->parent_page_offset; // 새로운 page에 offset 할당
    new_key = nl->records[0].key;

    pwrite(fd, nl, sizeof(page), new_leaf); // page들을 write
...
int db_delete(int64_t key)
{
    ...
    if (rt->num_of_keys == 0) // tree에 node가 없는 경우
    {
        // printf("root is empty\n");
        return -1;
    }
    char *check = db_find(key); // key를 통해 해당 value를 찾는다
    if (check == NULL)
    {
        free(check);
    ...
    free(check);
    off_t deloff = find_leaf(key); // delete할 record 가 존재하는 page의 off_t
    delete_entry(key, deloff);     // 실제 delete
    return 0;
...
void delete_entry(int64_t key, off_t deloff)
{
    ...
    remove_entry_from_page(key, deloff); // leaf 와 interanl의 case 나누어서 delete

    if (deloff == hp->rpo) // 현재 delete 한 page가 root page라면
    {
        adjust_root(deloff); // header page에 대한 정보 수정 & root page 변경
        return;
    }

    // 현재 delete 한 record는 leaf나 interanl
    page *not_enough = load_page(deloff);
    int check = not_enough->is_leaf ? cut(LEAF_MAX) : cut(INTERNAL_MAX); // 삭제를 한 page가 leaf? or internal?

    if (not_enough->num_of_keys >= check) // check는 일종의 threshold 조건 (통과)
    {
        free(not_enough);
        // printf("just delete\n");
        return;
    ...
    int64_t k_prime;
    parent_offset = not_enough->parent_page_offset; // merge를 하기 위해서는 parent page가 필요하다
    page *parent = load_page(parent_offset);

    // internal의 parent나 leaf의 parent나 모두 internal

    if (parent->next_offset == deloff) // 가장 오른쪽 자식 page == 삭제한 record가 있는 page
    {
        neighbor_index = -2;
    ...
    else if (parent->b_f[0].p_offset == deloff) // 가장 왼쪽 자식 page == 삭제한 record가 있는 page
    {
        neighbor_index = -1;
    ...
    else // 중간 자식 page == 삭제한 record가 있는 page
    {
        int i;

        for (i = 0; i <= parent->num_of_keys; i++)
            if (parent->b_f[i].p_offset == deloff) // i번째 자식 page(recod가 있는 page)
                break;
        neighbor_index = i - 1;
...
void adjust_root(off_t deloff)
{
    ...
    if (rt->num_of_keys > 0) // root key >=1  조건 성립
        return;
    if (!rt->is_leaf) // root key == 0  이면서 leaf 가 아닌경우 즉 height가 2보다 큰 경우
    {
        off_t nr = rt->next_offset;
        page *nroot = load_page(nr);
        nroot->parent_page_offset = 0;
        usetofree(hp->rpo); // 현재 root page의 free page로
        hp->rpo = nr;       // root page를 다음 root page로 변경
        pwrite(fd, hp, sizeof(H_P), 0);
        free(hp);
        hp = load_header(0);
    ...
    { // root key === 이면서 leaf인 경우 height == 0
      // 현재 root page를 free page
        free(rt);
        rt = NULL;
        usetofree(hp->rpo);
        hp->rpo = 0;
...
void remove_entry_from_page(int64_t key, off_t deloff)
{
    ...
    if (lp->is_leaf) // leaf라면
    {
        // printf("remove leaf key %ld\n", key);
        while (lp->records[i].key != key) //  delete할 record의 위치 찾기
            i++;

        for (++i; i < lp->num_of_keys; i++) // 해당 위치 뒤에 있는 record를 한칸 씩 당기기
            lp->records[i - 1] = lp->records[i];
        lp->num_of_keys--;
        pwrite(fd, lp, sizeof(page), deloff); //수정한 page write

        if (deloff == hp->rpo) // 수정한게 root page라면 (즉 root 이면서 leaf)
        {
            free(lp);
            free(rt);
            rt = load_page(deloff);
    ...
    else // leaf가 아니라면
    {
        ...
        for (++i; i < lp->num_of_keys; i++)
            lp->b_f[i - 1] = lp->b_f[i]; // internal recorde를 위와 같은 방식으로 당기기
        lp->num_of_keys--;
